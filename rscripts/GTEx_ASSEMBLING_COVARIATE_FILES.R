...
for (current_tissue in tissue_list) {
  ...
  current_tissue_cov <- fread(paste0(input_dir,"/covariates/",current_tissue,"_covariates_2-6-2021.csv"),header=T,sep=",")
  current_tissue_cov <- current_tissue_cov %>% select(CollaboratorParticipantID,SEX) %>% filter(CollaboratorParticipantID%in%mQTL_sample_list) %>% mutate(SEX = SEX-1)
