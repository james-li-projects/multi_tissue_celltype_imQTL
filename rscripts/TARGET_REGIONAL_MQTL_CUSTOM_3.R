...
candidate_str="colon_Stromal.cg21130420.chr2:113827728:A:G"
...
# Manually set y-axis limit for mQTL to 6
panel_max_df <- panel_max_df %>%
  mutate(y_max = ifelse(Ancestry == "mQTL", 6, y_max))
...
p <- ggplot(plot_df_final, aes(x = POS / 1e6, y = -log10(P))) +
  ...
  geom_blank(data = panel_max_df, aes(x = POS / 1e6, y = y_max)) +
