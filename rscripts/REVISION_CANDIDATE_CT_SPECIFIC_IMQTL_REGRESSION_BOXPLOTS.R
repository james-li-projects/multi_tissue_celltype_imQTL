...
#candidate_str="colon_Epi.cg07179016.chr1:1130902:A:G"
...
imqtl_requests <- fread("/gpfs/data/pierce-lab/james.li/imQTL/output/CANDIDATE_CT_SPECIFIC/tables/candidate_ct_specific_imqtl_DF.tsv") %>% rename(current_tissue=tissue) %>% select(-celltype) %>% filter(target_cpg%in%c("cg17775713","cg14111612")) 
...
for (group_key in names(grouped_requests)) {
  ...
  if (dataset == "GTEx") {
    ...
    input_dnam_bed <- fread(paste0("/gpfs/data/pierce-lab/james.li/imQTL/input/",dataset,"/",current_tissue,".bed"),sep="\t") %>% select(`#chr`,start,end,phenotype_id)
    
    # importing tissue DNA methylation bed file with beta values
    loadRData <- function(fileName){
      load(fileName)
      get(ls()[ls() != "fileName"])
    }
    noob_final_BMIQ <- loadRData(paste0("/gpfs/data/pierce-lab/james.li/imQTL/data/GTEx/methylation/noob_final_BMIQ_",current_tissue,"_2-6-2021.RData"))
    library(data.table)
    setDT(input_dnam_bed)
    setkey(input_dnam_bed, phenotype_id)
    meth_dt <- as.data.table(
      noob_final_BMIQ,
      keep.rownames = "phenotype_id"
    )
    setkey(meth_dt, phenotype_id)
    combined_bed <- input_dnam_bed[meth_dt, nomatch = 0]
    # ensure column order is correct for bed file
    setcolorder(
      combined_bed,
      c("#chr", "start", "end", "phenotype_id",
        setdiff(names(combined_bed), c("#chr", "start", "end", "phenotype_id")))
    )
    input_dnam_bed <- combined_bed
    rm(combined_bed)
    # making sample names consistent with other data sources
    library(data.table)
    sample_cols <- names(input_dnam_bed)[5:ncol(input_dnam_bed)]
    setnames(
      input_dnam_bed,
      old = sample_cols,
      new = sub("^([^-]+-[^-]+)-.*$", "\\1", sample_cols)
    )
    ...
    noob_final_BMIQ <- loadRData(paste0("/gpfs/data/phs/groups/Projects/GEMS/james.li/EWAS/manuscript/data/combined_beta.RData"))
    library(data.table)
    setDT(input_dnam_bed)
    setkey(input_dnam_bed, phenotype_id)
    meth_dt <- as.data.table(
  ...
  for (i in seq_len(nrow(request_group))) {
    ...
    for (tmp_combination in tmp_combination_list) {
      ...
      # parse the cpg bed df if HEALS prior to joining with other data sources
      if (dataset=="HEALS") {
        library(dplyr)
        lookup <- cov_df %>%
          transmute(
            individual_id = sub("^[^_]+_", "", Sample_Name),
            Sample_Name_full = Sample_Name
          )
        cpg_df <- cpg_df %>%
          mutate(individual_id = sub("^X", "", Sample_Name)) %>%
          left_join(lookup, by = "individual_id") %>%
          mutate(Sample_Name = Sample_Name_full) %>%
          select(-individual_id, -Sample_Name_full)
      }
      
      # creating regression df 
      reg_df <- cpg_df %>%
    ...
    # Step 1: Reformat facet labels to two lines
    all_ct_df <- all_ct_df %>%
      mutate(`Cell Type Proportion` = gsub("Upper 50% \\(([^)]+)\\)", "Upper 50%\n\\1", `Cell Type Proportion`))
    
    # Step 2: Safely compute beta and p-value for each facet
    facet_stats <- all_ct_df %>%
      group_by(`Cell Type Proportion`) %>%
      summarise(
        beta = tryCatch(coef(lm(mval ~ Genotype_numeric))[["Genotype_numeric"]], error = function(e) NA),
        pval = tryCatch(summary(lm(mval ~ Genotype_numeric))$coefficients["Genotype_numeric", "Pr(>|t|)"], error = function(e) NA),
        .groups = "drop"
      ) %>%
      mutate(
        beta_fmt = ifelse(is.na(beta), "NA", sprintf("%.2f", beta)),
        pval_fmt = ifelse(
          is.na(pval), "NA",
          ifelse(pval < 0.05, formatC(pval, format = "e", digits = 1), sprintf("%.2f", pval))
        ),
        label = paste0("\u03B2 = ", beta_fmt, "\np = ", pval_fmt)
      )
    
    # Step 3: Fix annotation position (same across panels)
    fixed_y <- min(all_ct_df$mval, na.rm = TRUE)
    fixed_x <- max(as.numeric(all_ct_df$Genotype_numeric), na.rm = TRUE)
    
    label_pos_df <- facet_stats %>%
      mutate(
        x = fixed_x,
        y = fixed_y
      )
    ...
    if (any(nchar(c(hom_rec_str, het_str, hom_dom_str)) > 5)) {
      interact_boxplot_p <- ggplot(all_ct_df, aes(x = Genotype, y = mval)) +
        ...
        geom_text(data = label_pos_df, aes(x = x, y = y, label = label), hjust = 1, vjust = 0, size = 4) +
        facet_wrap(~`Cell Type Proportion`, nrow = 1) +
        theme_classic() +
        labs(
          x = "Genotype", y = "DNAm (beta value)",
      ...
      out_dir <- paste0("/gpfs/data/pierce-lab/james.li/imQTL/output/CANDIDATE_CT_SPECIFIC/plots/boxplots_revision/", current_tissue)
      dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
      ggsave(paste0(out_dir, "/", lead_variant_period, "_", target_cpg, ".png"), interact_boxplot_p, width = 12, height = 4.35)
    } else {
      ...
      interact_boxplot_p <- ggplot(all_ct_df, aes(x = Genotype, y = mval)) +
        ...
        labs(
          x = "Genotype", y = "DNAm (beta value)",
          title = paste0("DNAm at ",target_cpg," in ",current_tissue," by ",lead_variant_colon," genotype")) +
        theme(
          axis.text = element_text(size = 14),
          axis.title = element_text(size = 16),
          strip.text = element_text(size = 14),
      ...
      out_dir <- paste0("/gpfs/data/pierce-lab/james.li/imQTL/output/CANDIDATE_CT_SPECIFIC/plots/boxplots_revision/", current_tissue)
      dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
      ggsave(paste0(out_dir, "/", lead_variant_period, "_", target_cpg, ".png"), interact_boxplot_p, width = 12, height = 4)
    }
