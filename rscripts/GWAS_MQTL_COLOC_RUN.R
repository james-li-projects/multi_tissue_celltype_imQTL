...
setwd("/gpfs/data/pierce-lab/james.li/imQTL/output/GWAS_coloc/extract_mqtl_data")
...
for (i in 1:length(imqtl_file_list)) {
  ...
  print(paste("Performing coloc on mqtl",i,"out of",total_num_imqtl))
  
  # importing file argument for current imQTL file
  tmp_mqtl_file <- imqtl_file_list[i]
  
  # parsing out relevant strings from imQTL filename
  tmp_cleaned <- sub("\\.tsv$", "", tmp_mqtl_file)
  parts <- strsplit(tmp_cleaned, "_")[[1]]
  tmp_mqtl_combination <- paste(parts[1:2], collapse = "_")
  tmp_mqtl_cpg_id <- parts[3]
  tmp_mqtl_variant_id <- paste(parts[4:7], collapse = ":")
  
  ##################################
  # IMPORTING AND PROCESSING IMQTL #
  ##################################
  # importing imqtl DF
  tmp_mqtl <- fread(paste0("/gpfs/data/pierce-lab/james.li/imQTL/output/GWAS_coloc/extract_mqtl_data/",tmp_mqtl_file))
  tmp_mqtl <- tmp_mqtl %>%
    mutate(
  ...
  tmp_common_variant_list <- intersect(tmp_GWAS$variant_id,tmp_mqtl$variant_id)
  
  # identifying if the lead variant is in both datasets
  in_gwas   <- tmp_mqtl_variant_id %in% tmp_GWAS$variant_id
  in_mqtl  <- tmp_mqtl_variant_id %in% tmp_mqtl$variant_id
  
  # if more than 50 variants shared between both datasets and the lead variant is in both datasets, continue processing for running coloc
  if (length(tmp_common_variant_list) > 50 && in_gwas && in_mqtl) {
    ...
    dataset2 <- tmp_mqtl %>%
      filter(variant_id %in% tmp_common_variant_list) %>%
      mutate(
        snp = variant_id,
        beta = slope,
        varbeta = slope_se^2,
...
write.table(coloc_results,file=paste0("/gpfs/data/pierce-lab/james.li/imQTL/output/GWAS_coloc/coloc_output_mqtl/",gsub("/gpfs/data/pierce-lab/james.li/GWAS_Trait_87/","",gsub(".txt.gz","",tmp_GWAS_file)),".tsv"),quote=F,row.names=F,col.names=T,sep="\t")
