...
all_imqtl_results <- all_imqtl_results %>% filter(class=="Interaction Effect")
...
cb_palette <- c("Not reproducible" = "#0072B2", "Reproducible" = "#E69F00")
...
distance_counts <- all_joined_df_binned %>%
  ...
  mutate(
    ...
    validated_label = ifelse(validated_status, "Reproducible", "Not reproducible")
  )
...
pie_data_combined <- bind_rows(pie_data, pie_data_all) %>% filter(celltype!="Monocyte") %>%  mutate(
    ...
    validated_label = factor(validated_label, levels = c("Reproducible", "Not reproducible"))
  )

cb_palette <- c("Reproducible" = "#E69F00", "Not reproducible" = "#0072B2")
...
p <- ggplot(pie_data_combined, aes(x = "", y = percent, fill = validated_label)) +
  ...
  scale_fill_manual(values = cb_palette) +
  labs(fill = "Reproducibility of Interaction Term") +
