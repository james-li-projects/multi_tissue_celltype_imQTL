...
candidate_str="lung_Mono.cg20356878.chr3:121995837:G:A"
query_ensg="ENSG00000114013.15"
query_rsid="rs9816720"
query_gwas_file="/gpfs/data/pierce-lab/james.li/GWAS_Trait_87/imputed_UKB_6152_9_diagnosed_by_doctor_Hayfever_allergic_rhinitis_or_eczema.txt.gz"
...
# importing eqtl data
data_eqtl <- fread(paste0("/gpfs/data/pierce-lab/james.li/imQTL/data/ADDITIONAL/GTEx_eQTL_v8/output/tsv/tensorQTL_eQTL_",query_tissue,".cis_qtl_pairs.chr",query_chr_int,".tsv")) %>% filter(phenotype_id==query_ensg) %>% as.data.table()
data_eqtl[, c("CHROM", "POS", "REF", "ALT") := tstrsplit(variant_id, ":", fixed=TRUE)]
data_eqtl[, POS := as.integer(POS)]
data_eqtl <- data_eqtl[abs(POS - query_pos) <= 250000]
data_eqtl <- data_eqtl %>% rename(P=pval_nominal) %>% select(variant_id,CHROM,POS,P) %>% mutate(Ancestry="eQTL") %>% rename(ID=variant_id)
...
#######################################
# COMPUTING LD WITH LEAD VARIANT EQTL #
#######################################
# writing out variant list 
write.table(rbind((data_eqtl%>%select(ID)),data.frame(ID=c(query_variant)))%>%unique(),file="/scratch/jll1/tmp/variant.list",quote=F,row.names=F,col.names=F)
# computing LD
max_snp_window<-nrow(data_eqtl)
system(paste0("module load plink/1.9; plink -bfile /gpfs/data/pierce-lab/james.li/imQTL/data/GTEx/genetic_data/processed_genetic_data_chrprefix_bfile --extract /scratch/jll1/tmp/variant.list --ld-snp ",query_variant," --ld-window-kb 500 --r2 --ld-window-r2 0 --ld-window ",max_snp_window," --memory 100000 --out /scratch/jll1/tmp/tmp_LD"))
# import pairwise correlations
pairwise_cor <- fread("/scratch/jll1/tmp/tmp_LD.ld")
# Filter the rows where either SNP_A or SNP_B equals query_variant
...
r2_df <- rbind(r2_df,data.frame(ID=query_variant,r2=1))
# joining correlations with main df
tmp_data_eqtl <- left_join(data_eqtl,r2_df,by=c("ID"))
data_eqtl <- tmp_data_eqtl

######################
# RBIND ALL PLOT DFS #   
######################
plot_df <- rbind(data_imqtl,data_gwas,data_eqtl)
...
plot_df <- plot_df %>%
  mutate(
    Ancestry = factor(Ancestry, levels = c("imQTL", "GWAS", "eQTL")),
...
p <- ggplot(plot_df_final, aes(x = POS / 1e6, y = -log10(P))) +
  ...
  labs(x = paste("Position on Chromosome", gsub("chr", "", query_chr), "(Mb)")) +
  theme_classic() +
  theme(
    ...
    legend.margin = margin(0.5, 2, 4, 2),
...
# Save the plot
output_path <- paste0(
  "/gpfs/data/pierce-lab/james.li/imQTL/output/target_figure_regional_plots/imqtl_gwas_eqtl/",
