...
extract_col_id=paste0("GTEx_",unique((wide_parsed_imqtl %>% filter(tissue=="colon"))$combination))

# filtering for tissue of interest in this DF and sig_Bhat
wide_parsed_imqtl <- wide_parsed_imqtl %>% filter(combination=="colon_Lym") %>% mutate(pair_id=paste(phenotype_id,variant_id,sep="_"),extract_col_id=paste0("GTEx_",tissue,"_",celltype))
...
flip_rows <- major_celltype_beta_df[["Colon - Lymphocyte"]] < 0
major_celltype_beta_df[flip_rows, ] <- -beta_df[flip_rows, ]
major_celltype_beta_df[["Colon - Lymphocyte"]] <- abs(major_celltype_beta_df[["Colon - Lymphocyte"]])
...
long_df <- major_celltype_beta_df %>%
  ...
  mutate(cell_type = gsub("Colon - ", "", cell_type))
