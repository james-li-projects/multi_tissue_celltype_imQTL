  ...
  for (current_tissue in tissue_list) {
    ...
    for (tmp_combination in tmp_combination_list) {
      ...
      for (b in seq_len(nrow(tmp_imqtl_df))) {
        ...
        # remove all other cell type columns
        combo_cov_df <- combo_cov_df %>% select(-starts_with("CT_"))
  
        # assembling the regression df      
        reg_df <- cpg_df %>%
...
write.table(all_results,file="/gpfs/data/pierce-lab/james.li/imQTL/tmp/all_results_ORIGINALcol_RAWct.tsv",quote=F,row.names=F,col.names=T,sep="\t")
