...
for (i in 1:length(imqtl_file_list)) {
  ...
  in_gwas   <- tmp_imqtl_variant_id %in% tmp_GWAS$variant_id
  in_imqtl  <- tmp_imqtl_variant_id %in% tmp_imqtl$variant_id
  ...
  if (length(tmp_common_variant_list) > 50 && in_gwas && in_imqtl) {
...
write.table(coloc_results,file=paste0("/gpfs/data/pierce-lab/james.li/imQTL/output/GWAS_coloc/coloc_output_custom_prior/",gsub("/gpfs/data/pierce-lab/james.li/GWAS_Trait_87/","",gsub(".txt.gz","",tmp_GWAS_file)),".tsv"),quote=F,row.names=F,col.names=T,sep="\t")
