  ...
  for (m in 1:length(tissue_list)) {
    ...
    if (nrow(tmp_eqtl>50)) {
      ...
      for (k in seq_along(phenotype_ids)) {
        ...
        in_eqtl   <- tmp_imqtl_variant_id %in% tmp_eqtl_subset$variant_id
        ...
        if (length(shared_variants) > 50 && in_eqtl && in_imqtl) {
          ...
          # Run coloc
          coloc_out <- coloc.abf(coloc_input1, coloc_input2)
...
output_path <- gsub("/gpfs/data/pierce-lab/james.li/imQTL/output/GWAS_coloc/extract_imqtl_list/","/gpfs/data/pierce-lab/james.li/imQTL/output/EQTL_coloc/coloc_output/",imported_imqtl_query_name)
