...
wide_parsed_imqtl <- wide_parsed_imqtl %>% filter(tissue=="lung") %>% mutate(pair_id=paste(phenotype_id,variant_id,sep="_"),extract_col_id=paste0("GTEx_",tissue,"_",celltype))
...
colnames(rounded_mat) <- gsub("Lung - ","",colnames(rounded_mat))
rownames(rounded_mat) <- gsub("Lung - ","",rownames(rounded_mat))

# Save high-quality PNG heatmap
png(filename=paste0("/gpfs/data/pierce-lab/james.li/imQTL/output/cross_tissue_compare/cross_tissue_compare_output/prop_shared_effects_lung.png"),width = 5.5, height = 5.5, unit="in", res = 1200)  # High resolution
# Plot
