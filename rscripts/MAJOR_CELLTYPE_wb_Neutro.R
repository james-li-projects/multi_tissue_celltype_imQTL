...
extract_col_id=paste0("HEALS_",unique((wide_parsed_imqtl %>% filter(tissue=="wb"))$combination))

# filtering for tissue of interest in this DF and sig_Bhat
wide_parsed_imqtl <- wide_parsed_imqtl %>% filter(combination=="wb_Neutro") %>% mutate(pair_id=paste(phenotype_id,variant_id,sep="_"),extract_col_id=paste0("HEALS_",tissue,"_",celltype))
...
selected_columns <- c(
  "HEALS_wb_B", "HEALS_wb_CD4T", "HEALS_wb_CD8T", "HEALS_wb_Mono", "HEALS_wb_NK", "HEALS_wb_Neutro"
)
...
flip_rows <- major_celltype_beta_df[["Whole Blood - Neutrophil"]] < 0
major_celltype_beta_df[flip_rows, ] <- -beta_df[flip_rows, ]
major_celltype_beta_df[["Whole Blood - Neutrophil"]] <- abs(major_celltype_beta_df[["Whole Blood - Neutrophil"]])
...
long_df <- major_celltype_beta_df %>%
  ...
  mutate(cell_type = gsub("Whole Blood - ", "", cell_type))
...
p <- ggplot(long_df, aes(x = cell_type, y = effect_size)) +
  ...
  scale_y_continuous(breaks = seq(
    floor(min(long_df$effect_size, na.rm = TRUE) * 5) / 5,
    ceiling(max(long_df$effect_size, na.rm = TRUE) * 5) / 5,
    by = 0.2
  ))
