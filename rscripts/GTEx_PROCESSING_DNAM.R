...
celltype_dir <- "/gpfs/data/pierce-lab/james.li/imQTL/output/GTEx/cell_type_frac"
...
for (i in 1:7) {
  ...
  transform_INT_noob_final_BMIQ <- t(apply(X=mval,MARGIN=1, FUN=RankNorm))
  
  # generating and outputting DNAm bed file
  print(paste("Outputting DNAm .BED file"))
  transform_INT_noob_final_BMIQ_df <- data.frame(transform_INT_noob_final_BMIQ)
  # modifying column names to have dashes instead of periods
  colnames(transform_INT_noob_final_BMIQ_df) <- gsub("\\.","-",colnames(transform_INT_noob_final_BMIQ_df))
  # arranging matrix by sample list order
  transform_INT_noob_final_BMIQ_df <- transform_INT_noob_final_BMIQ_df[,mQTL_sample_list]
  transform_INT_noob_final_BMIQ_df$phenotype_id<-rownames(transform_INT_noob_final_BMIQ_df)
  current_tissue_BED <- inner_join(bed_label,transform_INT_noob_final_BMIQ_df, by = c("phenotype_id")) 
  ...
  prcompResult<-prcomp(t(transform_INT_noob_final_BMIQ),center=TRUE,scale.=TRUE)
  PCs<-prcompResult$x
